        ...
        <div class="card-flat rounded-2xl overflow-hidden mt-6">
            <form method="GET" action="{{ route('hr.attendance.index') }}"
                ...
                <div>
                    ...
                    <p class="text-xs text-on-surface-variant/50 mt-0.5">{{ count($logs) }} karyawan tercatat · klik foto
                            ...
                            <tr class="hover:bg-primary/5 transition">
                                <td class="px-6 py-3.5">
                                    <div class="flex flex-col">
                                        ...
                                        <p class="text-[10px] text-on-surface-variant/50 line-clamp-1 mt-0.5">
                                            <span
                                                class="font-semibold border-r border-black/10 pr-1.5 mr-1.5">{{ $l['nip'] }}</span>
                                            {{ $l['loc'] }}
                                        </p>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3.5 font-mono-data text-xs text-on-surface">
                                    <span class="font-bold">{{ $l['in'] }}</span> <span
                                        class="text-on-surface-variant/30">–</span> <span
                                        class="font-bold">{{ $l['out'] }}</span>
                                </td>
                                ...
                                <td class="px-4 py-3.5">
                                    <span
                                        class="text-[11px] font-bold px-2.5 py-1 rounded {{ $badge[$l['status']] ?? 'bg-gray-200 text-gray-700' }}">{{ $l['status'] }}</span>
                                </td>
                                <td class="px-6 py-3.5">
                                    @if ($l['attachment'])
        ...
        @foreach ($logs as $l)
            @if ($l['in'] !== '-')
                <div x-show="activePreview === {{ $loop->index }}" x-cloak
                    class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" @click.self="activePreview = null">
                    <div
                        class="bg-white rounded-2xl overflow-hidden max-w-sm w-full shadow-2xl animate-in fade-in zoom-in-95 duration-150">
                        <div class="relative">
                            ...
                            <div
                                class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent px-4 pt-10 pb-3.5 text-white">
                                ...
                                <div class="flex items-center gap-2 mt-1 text-[10px] text-white/70">
                                    ...
                                    <span
                                        class="px-1.5 py-0.5 rounded {{ $l['mock_gps'] ? 'bg-error/80' : 'bg-primary/80' }} text-white">
