...
@php
    $categoryColor = [
        ...
        'Pendapatan Variabel' => 'bg-gray-50/10 text-gray-700',
...
<div x-data="{
    ...
    <div class="grid grid-cols-4 gap-5">
        <div class="card-flat rounded-md p-5">
            <p class="text-xs font-medium text-on-surface-variant/50 uppercase tracking-wide mb-1">Total Komponen Aktif</p>
            <p class="text-2xl font-semibold -data text-on-surface">{{ $totalComponents }}</p>
            <p class="text-[11px] text-on-surface-variant/40 mt-1">Master komponen sistem</p>
        </div>
        <div class="card-flat rounded-md p-5">
            <p class="text-xs font-medium text-on-surface-variant/50 uppercase tracking-wide mb-1">Pendapatan Tetap</p>
            <p class="text-2xl font-semibold -data text-primary">{{ $totalTetap }}</p>
            <p class="text-[11px] text-on-surface-variant/40 mt-1">Fixed basis kontrak</p>
        </div>
        <div class="card-flat rounded-md p-5">
            <p class="text-xs font-medium text-on-surface-variant/50 uppercase tracking-wide mb-1">Pendapatan Variabel</p>
            <p class="text-2xl font-semibold -data text-gray-700">{{ $totalVariabel }}</p>
            <p class="text-[11px] text-on-surface-variant/40 mt-1">Harian, SPL, &amp; klaim</p>
        </div>
        <div class="card-flat rounded-md p-5">
            <p class="text-xs font-medium text-on-surface-variant/50 uppercase tracking-wide mb-1">Potongan Wajib</p>
            <p class="text-2xl font-semibold -data text-error">{{ $totalPotongan }}</p>
            <p class="text-[11px] text-on-surface-variant/40 mt-1">BPJS &amp; PPh21 TER</p>
        </div>
    </div>


    <div class="card-flat rounded-md overflow-hidden mt-6">
        <div class="px-6 py-4 border-b border-black/5 flex items-center justify-between gap-4 flex-wrap">
            <div>
                <h2 class="text-base font-medium text-on-surface">Master Komponen &amp; Rumus Gaji</h2>
            ...
            <div class="flex items-center gap-2.5">
                <button type="button" @click="showAddModal = true"
                        class="bg-primary hover:brightness-110 text-white text-xs font-medium px-4 py-2.5 rounded-lg flex items-center gap-1.5 shadow-sm transition">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-surface-container text-left text-[11px] font-medium text-on-surface-variant/50 uppercase tracking-widest border-b border-black/5">
                        ...
                        <tr class="hover:bg-primary/5 transition">
                            <td class="px-6 py-3.5">
                                ...
                                <div class="flex items-center gap-1.5 mt-0.5">
                                    <span class="text-[10px] -data text-on-surface-variant/40">
                                        {{ $c->type === 'earning' ? 'Earning' : 'Deduction' }}
                                    </span>
                                    @if ($c->is_taxable)
                                        <span class="text-[9px] font-medium px-1.5 py-0.5 rounded bg-error/10 text-error">Kena Pajak</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-4 py-3.5">
                                <span class="text-[11px] font-medium px-2.5 py-1 rounded {{ $categoryColor[$c->category] ?? 'bg-surface-container text-on-surface-variant/60' }}">
                                    {{ $c->category ?? '-' }}
                                </span>
                            </td>
                            <td class="px-4 py-3.5 text-on-surface-variant/70 text-xs -data">{{ $c->calculation_type ?? '-' }}</td>
                            <td class="px-4 py-3.5 -data text-xs text-on-surface font-semibold">
    ...
    <div x-show="showAddModal" x-cloak
        ...
        <div class="bg-white rounded-md max-w-md w-full p-6 shadow-sm space-y-4">
            <div class="flex items-center justify-between border-b border-black/5 pb-3">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-medium text-on-surface">Tambah Komponen Gaji</h3>
                </div>
            ...
            <form method="POST" action="{{ route('hr.payroll.components.store') }}" class="space-y-3 text-xs">
                @csrf
                <div>
                    <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Nama Komponen</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Kategori</label>
                    ...
                    <div>
                        <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Tipe (untuk kalkulasi)</label>
                ...
                <div>
                    <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Tipe Kalkulasi (tampilan)</label>
                    <input type="text" name="calculation_type" placeholder="Contoh: Fixed Nominal, Harian, Persentase"
                           class="w-full border border-black/10 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-primary/20 focus:outline-none">
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Nominal Default</label>
                        <input type="number" step="0.01" name="default_amount" placeholder="250000"
                               class="w-full border border-black/10 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-primary/20 focus:outline-none">
                    </div>
                    <div class="flex items-end pb-2.5">
                        <label class="flex items-center gap-2 text-xs font-medium text-on-surface-variant/70">
                ...
                <div>
                    <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Catatan Rumus</label>
                    <input type="text" name="formula_note" placeholder="Contoh: 1/173 × Gaji Pokok × Jam"
                           class="w-full border border-black/10 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-primary/20 focus:outline-none">
                </div>

                <div>
                    <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Keterangan (opsional)</label>
                ...
                <div class="flex items-center justify-end gap-2.5 pt-2 border-t border-black/5">
                    <button type="button" @click="showAddModal = false"
                            class="px-4 py-2.5 rounded-lg border border-black/10 text-xs font-medium text-on-surface-variant/70 hover:bg-surface-container transition">
                        Batal
                    </button>
                    <button type="submit"
                            class="px-5 py-2.5 rounded-lg bg-primary text-white text-xs font-medium hover:brightness-110 shadow-sm transition">
                        Simpan Komponen
                    </button>
                </div>
            </form>
    ...
    <div x-show="showEditModal" x-cloak
        ...
        <div class="bg-white rounded-md max-w-md w-full p-6 shadow-sm space-y-4" x-show="editing">
            <div class="flex items-center justify-between border-b border-black/5 pb-3">
                <div class="flex items-center gap-2">
                    <span class="material-symbols-outlined text-primary text-[20px]">edit</span>
                    <h3 class="text-base font-medium text-on-surface">Edit Komponen Gaji</h3>
                </div>
            ...
            <form :action="editing ? '{{ url('hr/penggajian/komponen') }}/' + editing.id : '#'" method="POST" class="space-y-3 text-xs">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Tipe</label>
                        <select name="type" required class="w-full border border-black/10 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-primary/20 focus:outline-none">
                ...
                <div>
                    <label class="font-medium text-on-surface-variant/70 uppercase block mb-1">Tipe Kalkulasi</label>
    ...
    <div x-show="toast.show" x-transition
         class="fixed bottom-6 right-6 z-50 bg-[#0B3D2E] text-white text-xs font-medium px-4 py-2.5 rounded-md flex items-center gap-2 shadow-sm">
