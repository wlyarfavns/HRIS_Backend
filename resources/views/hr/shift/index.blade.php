    ...
    <div x-data="{
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            @forelse ($shiftTypes as $t)
                <div
                    class="bg-white rounded-2xl p-5 border border-black/5 shadow-sm hover:shadow-md transition space-y-2 relative overflow-hidden">
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="text-[11px] font-bold px-2 py-0.5 rounded-full bg-black/5 text-on-surface-variant/70">{{ $t['count'] }}
                            Karyawan</span>
                    </div>
            ...
            @empty
                <div
                    class="col-span-4 p-6 rounded-2xl border border-dashed border-black/10 text-center text-xs text-on-surface-variant/50">
        ...
        <div class="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-2.5">
                            ...
                            <span
                                class="text-xs font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">Periode
                                Aktif</span>
                        </div>
                    ...
                    <button type="button" @click="showBulkModal = true" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl
                ...
                <div class="flex items-center gap-3 flex-wrap">
                    <div class="relative">
                        <input type="text" x-model="searchQuery" placeholder="Cari karyawan / NIP..."
                            class="pl-9 pr-3 py-2 text-xs border border-black/10 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary w-44">
                        <span
                            class="material-symbols-outlined absolute left-2.5 top-1/2 -translate-y-1/2 text-on-surface-variant/50 text-[16px]">search</span>
                    </div>

                    <select x-model="selectedDept"
                        class="text-xs border border-black/10 rounded-xl pl-3 pr-8 py-2 bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary cursor-pointer font-medium">
                    ...
                    <div class="flex items-center border border-black/10 rounded-xl p-0.5 bg-surface-variant/10">
                        ...
                        <span class="text-xs font-bold px-2 text-on-surface">{{ $weekStart->translatedFormat('d M') }} -
                    ...
                    <div class="relative">
                        ...
                        <span
                            class="material-symbols-outlined absolute left-2.5 top-1/2 -translate-y-1/2 text-on-surface-variant/50 text-[16px] pointer-events-none">calendar_month</span>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            class="bg-surface-variant/20 border-b border-black/5 text-on-surface-variant/70 text-xs font-bold uppercase tracking-wider">
                            ...
                            @foreach ($labels as $l)
                                <th class="px-2 py-3.5 text-center min-w-[85px] border-l border-black/5">
                                    ...
                                    <span
                                        class="block text-[10px] font-normal text-on-surface-variant/60">{{ $l['date'] }}</span>
                                </th>
                            @endforeach
                        </tr>
                    </thead>
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ 2 + count($labels) }}"
                                    class="px-6 py-10 text-center text-xs text-on-surface-variant/50">
            ...
            <div
                class="px-6 py-3.5 bg-surface-variant/15 border-t border-black/5 flex items-center justify-between text-xs text-on-surface-variant/70 flex-wrap gap-4">
                <div class="flex items-center gap-4 flex-wrap">
                    @foreach ($shiftTypes as $t)
                        <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded {{ $t['bg'] }}"></span>
                            <strong>{{ $t['code'] }}</strong> = {{ $t['name'] }}</span>
                    @endforeach
                </div>
            ...
            <div class="bg-white rounded-2xl border border-black/5 p-6 shadow-sm space-y-4">
                <div class="flex items-center justify-between border-b border-black/5 pb-4">
                    <div>
                        ...
                        {{-- SESUDAH --}}
                        <p class="text-xs text-on-surface-variant/60 mt-0.5">Menunggu persetujuan HR</p>
                    </div>
                    @if ($pendingSpvCount > 0)
                        <span
                            class="text-xs font-bold px-2.5 py-1 rounded-full bg-amber-100 text-amber-800 border border-amber-200">{{ $pendingSpvCount }}
                            Menunggu SPV</span>
                    @endif
                </div>
                        ...
                        <div class="p-4 rounded-xl border border-black/10 bg-surface-variant/5 space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                @if ($req['status_raw'] === 'pending_spv')
                                    <span
                                        class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-800 border border-amber-200">Menunggu
                                        SPV</span>
                                @elseif ($req['status_raw'] === 'approved')
                                    <span
                                        class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">Disetujui</span>
                                @else
                                    <span
                                        class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-rose-100 text-rose-800 border border-rose-200">Ditolak</span>
                                @endif
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 text-xs">
                                <div class="p-2.5 rounded-xl bg-white border border-black/5">
                                    <span
                                        class="text-[10px] text-on-surface-variant/60 font-semibold uppercase block mb-0.5">Shift
                                        Semula</span>
                            ...
                            @if ($req['status_raw'] === 'pending_spv')
                                <div class="pt-2 flex justify-end gap-2">
                                    <form method="POST" action="{{ route('hr.shift.swap.reject', $req['db_id']) }}">
                                        @csrf
                                        <button type="submit"
                                            class="px-3 py-1.5 rounded-lg border border-black/10 text-xs font-semibold text-on-surface-variant hover:bg-black/5 transition">
                                            Tolak
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ route('hr.shift.swap.approve', $req['db_id']) }}">
                                        @csrf
                                        <button type="submit"
                                            class="px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold transition shadow-sm">
                                            Setujui
                                        </button>
                                    </form>
                                </div>
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between border-b border-black/5 pb-4">
                        <div>
                            ...
                            <p class="text-xs text-on-surface-variant/60 mt-0.5">Validasi absensi via GPS Haversine &
                                toleransi jam hadir</p>
                        </div>
                        <span
                            class="text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">GPS
                            Geotag Aktif</span>
                    </div>
                    ...
                    <div class="p-4 rounded-xl bg-surface-variant/10 border border-black/5 flex items-center gap-4">
                        <div
                            class="w-14 h-14 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center shrink-0 text-primary">
                    ...
                    <form method="POST" action="{{ route('hr.shift.geofencing.update') }}" class="space-y-4" x-data="{
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                <label class="text-xs font-bold text-on-surface-variant/80 uppercase tracking-wide">Titik
                        ...
                        <div class="space-y-3 pt-2">
                            <div class="flex items-center justify-between">
                                <label class="text-xs font-bold text-on-surface-variant/80 uppercase tracking-wide">Radius
                                    Absensi Kantor</label>
                                <span class="font-mono font-extrabold text-primary text-base"
                                    x-text="radiusMeter + ' meter'">100 meter</span>
                            </div>
                            <input type="range" name="geofence_radius_meters" min="25" max="500" step="25"
                                x-model="radiusMeter"
                                class="w-full accent-primary h-2 bg-black/10 rounded-lg cursor-pointer">
                        ...
                        <div class="grid grid-cols-2 gap-3 pt-3 border-t border-black/5">
                            <div class="p-3 rounded-xl border border-black/5 bg-surface-variant/10">
                                <label
                                    class="text-[10px] font-bold text-on-surface-variant/60 uppercase block mb-1.5">Toleransi
                                    Keterlambatan</label>
                                <div class="flex items-center gap-1.5">
                            ...
                            <div class="p-3 rounded-xl border border-black/5 bg-surface-variant/10">
                                <span class="text-[10px] font-bold text-on-surface-variant/60 uppercase">Logika Cross-Day
                                    (Malam)</span>
                                ...
                                <p class="text-[10px] text-on-surface-variant/50 mt-0.5">Diatur otomatis via kolom
        ...
        <div x-show="showBulkModal" x-cloak class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
            ...
            <form method="POST" action="{{ route('hr.shift.bulk-assign') }}"
                ...
                <div class="flex items-center justify-between border-b border-black/5 pb-3">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-on-surface-variant/60">Terapkan jadwal shift serentak ke banyak karyawan
                            </p>
                        </div>
                    </div>
                ...
                <div class="space-y-4 text-xs">
                    <div>
                        <label class="font-bold text-on-surface-variant/80 uppercase tracking-wide block mb-1.5">1. Pilih
                            Jenis Shift</label>
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            @foreach ($shiftTypes as $st)
                                <label
                                    class="border rounded-xl p-2.5 flex flex-col items-center gap-1 cursor-pointer transition text-center"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="font-bold text-on-surface-variant/80 uppercase tracking-wide block mb-1">2. Dari
                                Tanggal</label>
                        ...
                        <div>
                            <label class="font-bold text-on-surface-variant/80 uppercase tracking-wide block mb-1">Sampai
                                Tanggal</label>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="font-bold text-on-surface-variant/80 uppercase tracking-wide">3. Pilih Karyawan
                                Penerima</label>
                        ...
                        <div
                            class="max-h-36 overflow-y-auto border border-black/10 rounded-xl p-2 divide-y divide-black/5 bg-surface-variant/10">
                            @foreach ($roster as $r)
                                <label
                                    class="flex items-center justify-between p-2 hover:bg-white rounded-lg cursor-pointer transition">
                                    <div class="flex items-center gap-2.5">
                                        <input type="checkbox" name="employee_ids[]" value="{{ $r['id'] }}"
                                            x-model="selectedEmployees"
                                            class="rounded border-black/20 text-emerald-600 focus:ring-emerald-500/20">
                    ...
                    <div
                        class="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-900 flex items-start gap-2.5">
                        <span class="material-symbols-outlined text-[18px] text-amber-700 shrink-0 mt-0.5">info</span>
                        <p class="text-[11px] leading-relaxed">
                            <span class="font-bold">Overlap Shift Guard:</span> Sistem akan menimpa jadwal sebelumnya pada
                ...
                <div class="flex items-center justify-end gap-2.5 pt-3 border-t border-black/5">
                    ...
                    <button type="submit"
                        class="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-sm flex items-center gap-1.5 transition">
