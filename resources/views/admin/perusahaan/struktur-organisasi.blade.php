    ...
    <div class="bg-white rounded-xl p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">Klik departemen untuk lihat ringkasan posisi & karyawan</p>
            </div>
            <span class="text-[10px] font-bold font-mono-data text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full uppercase tracking-wide">Interaktif</span>
        </div>

        <div class="flex flex-col items-center gap-6" x-data="{ open: null }">
            <div class="bg-primary text-white text-sm font-semibold px-5 py-3 rounded-lg">
                CEO — Andi Wijaya
            </div>
                ...
                @foreach ($departments as $i => $dept)
                    <div class="flex flex-col items-center">
                        ...
                        <button @click="open = open === {{ $i }} ? null : {{ $i }}"
                                class="w-full text-center bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-3 hover:border-emerald-300 transition-colors">
        ...
        <div class="bg-white rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">{{ count($departments) }} departemen aktif</p>
                </div>
                <button class="bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold px-3.5 py-2.5 rounded-lg flex items-center gap-1.5 transition-colors">
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($departments as $dept)
                        <tr class="hover:bg-gray-50/60">
                            ...
                            <td class="px-6 py-3.5 text-center font-mono-data text-gray-900">{{ $dept['positions'] }}</td>
                            <td class="px-6 py-3.5">
                                <div class="flex items-center justify-center gap-1">
                                    <button title="Edit" class="p-1.5 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/5 transition-colors">
                                        <span class="material-symbols-outlined text-[18px]">edit</span>
                                    </button>
                                    <button title="Hapus" class="p-1.5 rounded-lg text-gray-400 hover:text-error hover:bg-error/5 transition-colors">
        ...
        <div class="bg-white rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">{{ count($jobGrades) }} grade terdaftar</p>
                </div>
                <button class="bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold px-3.5 py-2.5 rounded-lg flex items-center gap-1.5 transition-colors">
                    <span class="material-symbols-outlined text-[16px]">add</span>
                    Tambah
