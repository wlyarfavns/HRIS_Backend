...
<div class="grid grid-cols-3 gap-6 items-start">
    ...
    <div class="col-span-2 card-flat rounded-2xl overflow-hidden">
        ...
        <div class="px-8 py-6 text-white" style="background-color:#0B3D2E;">
            <div class="flex items-start justify-between">
                ...
                <div class="text-right">
                    <p class="text-sm font-bold">PT Talenta Digital Nusantara</p>
                    <p class="text-[11px] text-white/60 mt-0.5">Jl. Jenderal Sudirman No. 88, Jakarta Selatan</p>
                </div>
            </div>
        </div>
        ...
        <div class="px-8 py-6 border-b border-black/5 grid grid-cols-3 gap-5">
            <div class="col-span-1 flex items-center gap-3">
                <img src="https://i.pravatar.cc/48?img={{ $slip['avatar'] }}" class="w-12 h-12 rounded-full object-cover" alt="{{ $slip['name'] }}">
                <div>
        ...
        <div class="px-8 py-6 grid grid-cols-2 gap-8">
            <div>
                ...
                <div class="space-y-2.5">
                    @foreach ($slip['earnings'] as $e)
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-mono-data text-on-surface">{{ number_format($e['amount'], 0, ',', '.') }}</span>
                        </div>
                    @endforeach
                </div>
                <div class="flex items-center justify-between text-sm font-bold mt-3 pt-3 border-t border-black/5">
                    ...
                    <span class="font-mono-data text-primary">{{ number_format(collect($slip['earnings'])->sum('amount'), 0, ',', '.') }}</span>
                </div>
            </div>
            <div>
                ...
                <div class="flex items-center justify-between text-sm font-bold mt-3 pt-3 border-t border-black/5">
                    ...
                    <span class="font-mono-data text-error">-{{ number_format(collect($slip['deductions'])->sum('amount'), 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="px-8 py-5 flex items-center justify-end gap-3 border-t border-black/5">
            <button type="button" class="border border-black/10 text-on-surface-variant/70 text-xs font-bold px-4 py-2.5 rounded-lg flex items-center gap-1.5 hover:bg-surface-container transition">
    ...
    {{-- SIDEBAR: STATUS & LOG AKSES --}}
    <div class="space-y-5">
        <div class="card-flat rounded-2xl p-5">
            <p class="text-[11px] font-bold text-on-surface-variant/50 uppercase tracking-wide mb-3">Status Distribusi</p>
        ...
        <div class="card-flat rounded-2xl p-5">
            <p class="text-[11px] font-bold text-on-surface-variant/50 uppercase tracking-wide mb-3">Log Akses Karyawan</p>
            <div class="space-y-3">
                @foreach ($slip['access_log'] as $log)
                    <div class="flex items-start gap-2.5">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-on-surface-variant/40 font-mono-data">{{ $log['time'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
