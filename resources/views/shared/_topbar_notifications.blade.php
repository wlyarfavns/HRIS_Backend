    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
        ...
        <div
            ...
            <div class="flex items-center gap-2">
                ...
                <span x-show="unreadCount > 0" x-text="unreadCount + ' Baru'"
                    class="bg-primary/10 text-primary text-[10px] font-bold px-2 py-0.5 rounded-full"></span>
            </div>
        ...
        <div class="max-h-[360px] overflow-y-auto divide-y divide-black/5 custom-scrollbar">
            ...
            <template x-if="notifications.length === 0">
                <div class="px-5 py-8 text-center">
                    <span
                        class="material-symbols-outlined text-[32px] text-black/10 block mb-2">notifications_paused</span>
            ...
            <template x-for="(notif, index) in notifications" :key="notif.id">
                <a href="#" @click.prevent="readAndRedirect(notif.id, notif.data.url)"
                    ...
                    <!-- Icon Dinamis (bisa disesuaikan jika ingin warna berbeda, sementara pakai warna primary default) -->
                    <div
                    ...
                    <div class="flex-1 min-w-0 pr-4">
                        <p class="text-xs font-bold text-on-surface mb-0.5 group-hover:text-primary transition-colors"
        ...
        <div class="p-3 border-t border-black/5 bg-surface-container/20 text-center">
            <a href="{{ route('notifications.index') }}"
                class="inline-block text-xs font-bold text-primary hover:text-primary/80 transition-colors py-1">
                Lihat semua riwayat
            </a>
        </div>
    </div>
    ...
    function notificationDropdown() {
        return {
            ...
            formatDate(dateStr) {
                ...
                const now = new Date();
                const diff = (now - d) / 1000; // selisih dalam detik
